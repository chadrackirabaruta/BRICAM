...
<main id="main" class="main">
  <div class="pagetitle">
    <h1><i class="bi bi-person-plus"></i> Edit Customer: {{ $customer->name }}</h1>
  </div>
  ...
  <section class="section">
    <div class="card">
      <div class="card-body pt-4">
        <h5 class="card-title text-primary"><i class="bi bi-pencil-square"></i> Customer Information</h5>
        ...
        <form action="{{ route('customers.update', $customer->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <select name="customer_type" id="customer_type" class="form-select" required>
                ...
                <option value="Contractor" {{ old('customer_type', $customer->customer_type) == 'Contractor' ? 'selected' : '' }}>Contractor</option>
              </select>
            </div>

            <!-- Status -->
            <div class="col-md-6">
              <label for="status" class="form-label"><i class="bi bi-power"></i> Status <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
              ...
              <div class="input-group has-validation position-relative">
                <input type="text" name="id_number" id="id_number" class="form-control" maxlength="16" value="{{ old('id_number', $customer->id_number) }}">
            ...
            <div class="col-md-6">
              ...
              <div class="input-group position-relative">
                <input type="text" name="phone" id="phone" class="form-control" maxlength="10" value="{{ old('phone', $customer->phone) }}" required>
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="country" id="country" value="{{ old('country', $customer->country ?? 'Rwanda') }}" class="form-control" required>
            </div>

            <!-- Birth Date -->
            <div class="col-md-6">
              <label for="dob" class="form-label"><i class="bi bi-calendar2-day"></i> Date of Birth</label>
              <input type="date" name="dob" id="dob" class="form-control" value="{{ old('dob', $customer->dob ? $customer->dob->format('Y-m-d') : '') }}">
            </div>
            ...
            <div class="col-md-6">
              ...
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gender" id="male" value="Male" {{ old('gender', $customer->gender) == 'Male' ? 'checked' : '' }}>
                <label class="form-check-label" for="male">Male</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gender" id="female" value="Female" {{ old('gender', $customer->gender) == 'Female' ? 'checked' : '' }}>
                <label class="form-check-label" for="female">Female</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gender" id="other" value="Other" {{ old('gender', $customer->gender) == 'Other' ? 'checked' : '' }}>
            ...
            <div class="col-md-6">
              ...
              <div class="d-flex align-items-center">
                @if($customer->avatar)
                  <img src="{{ asset('storage/' . $customer->avatar) }}" class="rounded-circle me-3" width="50" height="50">
                @endif
          ...
          <div class="text-end mt-4">
            ...
            <button type="submit" class="btn btn-success"><i class="bi bi-check-circle"></i> Update Customer</button>
          </div>
        </form>
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", () => {
  // Get customer's current location values
  const customerLocation = {
    province: "{{ old('province', $customer->province) }}",
    district: "{{ old('district', $customer->district) }}",
    sector: "{{ old('sector', $customer->sector) }}",
    cell: "{{ old('cell', $customer->cell) }}",
  ...
  if (phoneInput.value.length > 0) {
    document.getElementById('phone-icon').style.display = 'inline-flex';
  ...
  fetch('/api/locations/provinces')
    ...
    .then(data => {
      ...
      data.forEach(prov => {
        const selected = prov === customerLocation.province ? 'selected' : '';
        provinceSelect.innerHTML += `<option value="${prov}" ${selected}>${prov}</option>`;
      });

      // If province is set, trigger district loading
      if (customerLocation.province) {
        provinceSelect.value = customerLocation.province;
  ...
  document.getElementById('province').addEventListener('change', function () {
    ...
    fetch(`/api/locations/districts/${val}`)
      ...
      .then(data => {
        ...
        data.forEach(d => {
          const selected = d === customerLocation.district && this.value === customerLocation.province ? 'selected' : '';
          district.innerHTML += `<option value="${d}" ${selected}>${d}</option>`;
        });

        // If district is set, trigger sector loading
        if (customerLocation.district && this.value === customerLocation.province) {
          district.value = customerLocation.district;
  ...
  document.getElementById('district').addEventListener('change', function () {
    ...
    fetch(`/api/locations/sectors/${val}`)
      ...
      .then(data => {
        ...
        data.forEach(s => {
          const selected = s === customerLocation.sector && this.value === customerLocation.district ? 'selected' : '';
          sector.innerHTML += `<option value="${s}" ${selected}>${s}</option>`;
        });

        // If sector is set, trigger cell loading
        if (customerLocation.sector && this.value === customerLocation.district) {
          sector.value = customerLocation.sector;
  ...
  document.getElementById('sector').addEventListener('change', function () {
    ...
    fetch(`/api/locations/cells/${val}`)
      ...
      .then(data => {
        ...
        data.forEach(c => {
          const selected = c === customerLocation.cell && this.value === customerLocation.sector ? 'selected' : '';
          cell.innerHTML += `<option value="${c}" ${selected}>${c}</option>`;
        });

        // If cell is set, trigger village loading
        if (customerLocation.cell && this.value === customerLocation.sector) {
          cell.value = customerLocation.cell;
  ...
  document.getElementById('cell').addEventListener('change', function () {
    ...
    fetch(`/api/locations/villages/${val}`)
      ...
      .then(data => {
        ...
        data.forEach(v => {
          const selected = v === customerLocation.village && this.value === customerLocation.cell ? 'selected' : '';
